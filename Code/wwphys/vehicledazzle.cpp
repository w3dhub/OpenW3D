...
int VehicleDazzleClass::Determine_Type(RenderObjClass * obj)
{
	if ((obj != NULL) && (obj->Class_ID() == RenderObjClass::CLASSID_DAZZLE)) {
        ...
        if (openw3d::string_len_compare(type_name,HEADLIGHT_NAME_PREFIX,strlen(HEADLIGHT_NAME_PREFIX)) == 0) {
			return HEADLIGHT_TYPE;
		}
		
        if (openw3d::string_len_compare(type_name,BRAKELIGHT_NAME_PREFIX,strlen(BRAKELIGHT_NAME_PREFIX)) == 0) {
			return BRAKELIGHT_TYPE;
		}

        if (openw3d::string_len_compare(type_name,BLINKLIGHT_NAME_PREFIX,strlen(BLINKLIGHT_NAME_PREFIX)) == 0) {
