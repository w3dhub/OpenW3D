...
"%d meshes have mesh anomalies associated with them. If the anomaly is serious, a\
 ...
 by left-clicking on the mesh name in the list.";
			...
			MessageBox (nullptr, anomalymessage.String(), "Attention", MB_ICONEXCLAMATION | MB_SETFOREGROUND);
		}
	...
	while (w3dchunk.Open_Chunk()) {
		...
		switch (w3dchunk.Cur_Chunk_ID()) {
			...
			case W3D_CHUNK_SHADER_IDS:
				if ((materialpass < MeshMatDescClass::MAX_PASSES) && (ShaderIdChunk [prelitmode][materialpass] == nullptr)) {
...
LightMapDoc::MeshInfoStruct::~MeshInfoStruct()
{
	...
	if (TriangleChunk != nullptr) delete TriangleChunk;
	if (VertexChunk != nullptr) delete VertexChunk;
	for (s = 0; s < PrelitModeEnum::COUNT; s++) {
		if (VertexMaterials [s] != nullptr) delete VertexMaterials [s];
		if (ShaderChunk [s] != nullptr) delete ShaderChunk [s];
		for (unsigned p = 0; p < MeshMatDescClass::MAX_PASSES; p++) {
			if (ShaderIdChunk [s][p] != nullptr) delete ShaderIdChunk [s][p];
		}
	}
}
...
LightMapDoc::MeshAnomalyStruct::MeshAnomalyStruct (const MeshInfoStruct &meshinfo)
{
	...
	if ((meshinfo.ShaderChunk [PrelitModeEnum::UNLIT] != nullptr) && (meshinfo.ShaderIdChunk [PrelitModeEnum::UNLIT] [0] != nullptr)) {
...
void LightMapDoc::Reorder()
{
	...
	try {
		...
		W3dFile->Close();
		if (reorderfile != nullptr) delete reorderfile;
...
void LightMapDoc::Optimize()
{
	...
	try {
		...
		W3dFile->Close();
		if (optimizefile != nullptr) delete optimizefile;
...
void LightMapDoc::Optimize_Prelit_Vertex_Material_Pass (ChunkLoadClass &w3dchunk, ChunkSaveClass &optimizechunk)
{
	...
	// Clean-up.
	if (dcgschunk != nullptr) delete dcgschunk;
	for (i = 0; i < digcount; i++) {
		if (digschunk [i] != nullptr) delete digschunk [i];
	}
}
...
{
	CFileDialog dialog (false, LightMapApp::Document_File_Extension(), nullptr, LightMapApp::File_Dialog_Flags(), LightMapApp::File_Dialog_Filter());
...
"%d meshes have solve anomalies associated with them. If the anomaly is serious, a\
 ...
 anomalies by left-clicking on the mesh name in the list.";
		...
		// Clean-up.
		if (meshfilebuffer != nullptr) delete [] meshfilebuffer;
		W3dFile->Close();
		if (solvefile != nullptr) delete solvefile;
