...
BOOL LightMapDoc::OnSaveDocument (LPCTSTR pathname) 
{
	...
	try {
		...
		if (IsModified() || openw3d::string_compare (W3dFile->File_Name(), pathname) != 0) {
		...
		while (loadchunk.Open_Chunk()) {
			...
			switch (loadchunk.Cur_Chunk_ID()) {
				...
				{	
					...
					if ((strlen (loaddrivename) == 0) && (openw3d::string_compare (loaddirectoryname, LightscapeSolve::Asset_Directory()) == 0)) {
