...
CW3DViewDoc::LoadAssetsFromFile (LPCTSTR lpszPathName)
{
	...
	if (::strrchr (lpszPathName, '\\')) {
		...
		stringTemp = stringTemp.Left (int(::strrchr (lpszPathName, '\\') - lpszPathName));
	...
	if (graphic_view != nullptr) {
		...
		if (camera != nullptr) {
			...
			if (m_ManualFOV) {
				...
				float hfov = ::strtof (hfov_string, nullptr);
				float vfov = ::strtof (vfov_string, nullptr);
			...
			if (m_ManualClipPlanes) {
				...
				float znear	= ::strtof (znear_string, nullptr);
				float zfar		= ::strtof (zfar_string, nullptr);
