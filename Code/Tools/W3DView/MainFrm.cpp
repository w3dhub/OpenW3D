...
)
{
	...
	if (bReturn) {
		...
		if (bReturn) {
			...
			BOOL bReturn2 = (pCGraphicView != nullptr);
            ...
            ASSERT (bReturn);
				...
				if (bReturn) {
					...
					if (::strstr (driver_name, "glide2") != nullptr) {
						...
						float driver_version = ::strtof (string_version, nullptr);
...
CMainFrame::OnAddToLineup (void)
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		else
		{
			...
			CString msg;
			msg.Format("Unable to create render object '%s'!", static_cast<const TCHAR*>(dlg.m_Object));
