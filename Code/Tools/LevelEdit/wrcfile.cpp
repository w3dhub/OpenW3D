...
WinResourceFileClass::~WinResourceFileClass(void)
{
	if (ResourceName)
		free(ResourceName);
}

char const * WinResourceFileClass::Set_Name(char const *filename)
{
	if (ResourceName) {
		free(ResourceName);
		ResourceName = NULL;
...
int WinResourceFileClass::Read(void *buffer, int size)
{
	if (!FilePtr) return 0;
...
int WinResourceFileClass::Seek(int pos, int dir)
{
	switch (dir) {
		case SEEK_SET:
