...
bool DX8Wrapper::Registry_Save_Render_Device( const char *sub_key, int device, int width, int height, int depth, bool windowed, int texture_depth)
{
	...
	ini.Put_String(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_NAME, _RenderDeviceShortNameTable[device]);
	ini.Put_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_WIDTH, width);
	ini.Put_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_HEIGHT, height);
	ini.Put_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_DEPTH, depth);
	ini.Put_Bool(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_WINDOWED, windowed != 0);
	ini.Put_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_TEXTURE_DEPTH, texture_depth);
...
bool DX8Wrapper::Registry_Load_Render_Device( const char * sub_key, char *device, int device_len, int &width, int &height, int &depth, int &windowed, int &texture_depth)
{
	...
	if (ini.Is_Present(OPENW3D_SECTION_RENDER)) {
		ini.Get_String(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_NAME, "", device, device_len);

		width =	ini.Get_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_WIDTH, -1);
		height = ini.Get_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_HEIGHT, -1);
		depth =	ini.Get_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_DEPTH, -1);
		windowed = ini.Get_Bool(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_WINDOWED, false);
		texture_depth = ini.Get_Int(OPENW3D_SECTION_RENDER, VALUE_INI_RENDER_DEVICE_TEXTURE_DEPTH, -1);
		return true;
