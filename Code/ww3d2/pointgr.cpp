...
{
	FrameRowColumnCountLog2 = std::min(frccl2, (unsigned char)4);
}
...
//	if (Texture) Texture->Process_Reduction();
   ...
   float value_255 = 0.9961f;	//254 / 255
	...
	while (current<vnum)
	{
		delta=std::min(vnum-current, MAX_VB_SIZE);
	...
	int &pnum)
{
	...
	if (VertexLoc.Length() < total_vnum) {
		...
		// OpenW3D @todo cfehunter 09/03/2025
		// These were passing false as the second parameter... I have no idea what they were trying to do.
