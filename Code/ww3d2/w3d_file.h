...
struct W3dRGBStruct
{
	...
	{
		R = (unsigned char)std::min(((unsigned)R) + ((unsigned)c.R), (unsigned)UCHAR_MAX);
		G = (unsigned char)std::min(((unsigned)G) + ((unsigned)c.G), (unsigned)UCHAR_MAX);
		B = (unsigned char)std::min(((unsigned)B) + ((unsigned)c.B), (unsigned)UCHAR_MAX);
		return (*this);
	}
