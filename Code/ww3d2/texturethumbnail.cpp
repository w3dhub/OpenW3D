...
static void Create_Hash_Name(StringClass& name, const StringClass& thumb_name)
{
	...
	WWASSERT(!openw3d::string_compare(&name[len-4],".tga") || !openw3d::string_compare(&name[len-4],".dds"));
	name[len-4]='\0';
}
...
void ThumbnailManagerClass::Create_Thumbnails()
{
	...
	if (mix.Is_Valid()) {
		...
		for (int i=0;i<list.Count();++i) {
			...
			if (!openw3d::string_compare(&list[i][len-4],".tga") || !openw3d::string_compare(&list[i][len-4],".dds")) {
