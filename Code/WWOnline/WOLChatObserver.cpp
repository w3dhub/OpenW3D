...
{
	if (openw3d::stricmp(UserName, whatever.UserName) == 0) {
		return(true);
	}
	return(false);
}			  
...
bool Is_Options_Spammer(char *user_name, int &count)
{
	...
	for (int i=0 ; i<OptionsTracking.Count() ; i++) {
		if (openw3d::stricmp(OptionsTracking[i]->UserName, user_name) == 0) {
			index = i;
			break;
		}
	}
	...
	*/
	...
	for (index = 0; index < count; index++) {
		...
		if (user.IsValid()) {
			...
			if (openw3d::stricmp((char*)wol_user->name, (char*)userdata.name) == 0) {
				ip = userdata.ipaddr;
				break;
			}
		}
