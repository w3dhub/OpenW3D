...
Protect::Protect()
	...
	mLauncherMutex = CreateMutexA(nullptr, false, LAUNCHER_GUID);
	...
	mMappedFile = CreateFileMappingA(INVALID_HANDLE_VALUE, &security,
		PAGE_READWRITE, 0, fileSize, nullptr);
...
void Protect::SendMappedFileHandle(HANDLE process, DWORD threadID) const
	...
	HANDLE event = CreateEventA(nullptr, false, false, protectGUID);
	...
	if (passKey.IsValid())
		...
		*/
		...
		if (result == ERROR_SUCCESS)
			...
			result = RegQueryValueExA(hKey, "InstallPath", nullptr, &type, installPath, &sizeOfBuffer);
			...
			BOOL volInfoSuccess = GetVolumeInformationA((const char*)drive, nullptr, 0,
			...
			*passKey += volumeSN;
			...
			result = RegQueryValueExA(hKey, "Serial", nullptr, &type, gameSerialNumber, &sizeOfBuffer);
		...
		if (result == ERROR_SUCCESS)
			...
			result = RegQueryValueExA(hKey, "ProductID", nullptr, &type, winProductID, &sizeOfBuffer);
...
void InitializeProtect(void)
	...
	mMappedFile = CreateFileMappingA(INVALID_HANDLE_VALUE, &security, PAGE_READWRITE, 0, fileSize, nullptr);
