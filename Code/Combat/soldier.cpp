		...
		for (int index = 0; index < obj_count; index ++) {
			...
			if (render_obj != NULL) {
				...
				if ( htree != NULL ) {
					...
					//
					if ( ::openw3d::stricmp (htree->Get_Name(), "S_A_HEAD") == 0 ||
						  ::openw3d::stricmp (htree->Get_Name(), "S_B_HEAD") == 0)
					{
		...
		if ( IS_MISSION ) {
			...
			if ( !Is_Human_Controlled() && visceroid_probability != 0 ) {
				if ( FreeRandom.Get_Float() < visceroid_probability ) {
					...
					if ( ::openw3d::stricmp( Get_Definition().Get_Name(), VISCEROID_NAME ) != 0 ) {
						// Create a visceroid
		...
		for ( int emitter = 0; emitter < NUM_SPECIAL_DAMAGE_EMITTERS; emitter++ ) {
			if ( _SpecialDamageEmitters[emitter].Mode == SpecialDamageMode ) {
				...
				while ( --sub_count >= 0 ) {
					...
					if ( ::openw3d::stricmp(sub->Get_Name(), _SpecialDamageEmitters[emitter].EmitterName ) == 0 ) {
...
{
	for ( int i = 0; i < RenderObjList.Count(); i++ ) {
		if ( !openw3d::stricmp( RenderObjList[i]->Get_Name(), name ) ) {
