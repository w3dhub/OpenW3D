...
CHECK_OPTIONAL_VARIABLE_NOPATH(GIT_BUILD_AUTHOR "")
...
#              with caution.
macro(RunGitCommand)
    execute_process(COMMAND ${CMAKE_COMMAND} -E env TZ=UTC 
        "${GIT_EXECUTABLE}" ${ARGV}
...
function(GetGitState _working_dir)
    ...
    if(exit_code EQUAL 0)
        set(ENV{GIT_REVISION_HASH} ${output})
    endif()

    RunGitCommand(show -s "--format=%h" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_REVISION} ${output})
    endif()

    RunGitCommand(show -s "--format=%cd" "--date=format-local:%Y-%m-%d %H:%M:%S" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMITTER_DATE} "${output}")
    endif()

    RunGitCommand(show -s "--format=%ct" ${object})
    if(exit_code EQUAL 0)
        set(ENV{GIT_COMMITTER_TIME_STAMP} "${output}")
    endif()

    # this command can fail if there are no tags on the repository.
    set(_permit_git_failure TRUE)
    RunGitCommand(describe --tags --match=v* --abbrev=0 ${object})
    set(_permit_git_failure FALSE)
    if(exit_code EQUAL 0)
        set(ENV{GIT_LATEST_TAG} ${output})
    endif()

    if (ENV{GIT_LATEST_TAG})
        RunGitCommand(rev-list "${GIT_LATEST_TAG}..${object}" --count)
        if(exit_code EQUAL 0)
            set(ENV{GIT_REVISION_COUNT} ${output})
        endif()

        if(ENV{GIT_LATEST_TAG} MATCHES "^v([0-9]+)\\.([0-9]+)\\.([0-9]+)(-.*)?$")
            set(ENV{GIT_VERSION_MAJOR} ${CMAKE_MATCH_1})
            set(ENV{GIT_VERSION_MINOR} ${CMAKE_MATCH_2})
            set(ENV{GIT_VERSION_PATCH} ${CMAKE_MATCH_3})
        else()
            message(WARNING "Tag doesn't follow SEMVER.")
            set(ENV{GIT_VERSION_MAJOR} 0)
            set(ENV{GIT_VERSION_MINOR} 0)
            set(ENV{GIT_VERSION_PATCH} 0)
        endif()
    else()
        ...
        if(exit_code EQUAL 0)
            ...
            set(ENV{GIT_REVISION_COUNT} 0)
        endif()

        set(ENV{GIT_VERSION_MAJOR} 0)
        set(ENV{GIT_VERSION_MINOR} 0)
        set(ENV{GIT_VERSION_PATCH} 0)
        set(ENV{GIT_LATEST_TAG} "v0.0.0")
    endif()
...
function(GitStateChangedAction)
    message("Running state change action.")
...
function(FakeGit)
    ...
    set(ENV{GIT_HAVE_INFO} "false")
    set(ENV{GIT_IS_DIRTY} "false")
    set(ENV{GIT_REVISION_HASH} "0123456789ABCFEF0123456789ABCFEF01234567")
    set(ENV{GIT_REVISION} "0123456")
    set(ENV{GIT_COMMITTER_DATE} "1970-01-01 00:00:00")
    set(ENV{GIT_COMMITTER_TIME_STAMP} 0)
    set(ENV{GIT_REVISION_COUNT} 0)
    set(ENV{GIT_VERSION_MAJOR} 0)
    set(ENV{GIT_VERSION_MINOR} 0)
    set(ENV{GIT_VERSION_PATCH} 0)
...
function(Main)
    if (GIT_EXECUTABLE AND GIT_INFO_ENABLED)
