...
)
{
	...
	if ( ::strlen( filename ) > 4 && ::openw3d::stricmp( filename + (::strlen( filename ) - 4), ".SAV" ) == 0 ) {
		return true;
	}

	// All save games not in the form Mxx.MIX are allowed
	if ( ( ::strlen( filename ) != 7 ) ||
		 ( ::openw3d::stricmp( filename + (::strlen( filename ) - 4), ".MIX" ) != 0 ) ||
		 ( ::toupper( *filename ) != 'M' ) ) {
		return true;
	}
...
LoadSPGameMenuClass::Update_Button_State (void)
{
	...
	if (item_index != -1) {
		...
		if (list_ctrl->Get_Entry_Data (item_index, 0) != NULL) {
			...
			if (len >= 4 && ::openw3d::stricmp ((filename.Peek_Buffer () + (len - 4)), ".mix") != 0) {
				enable = true;
			}
		}
