...
public:
	...
	virtual	void Activate( const char * input ) override {
		...
		if (!openw3d::stricmp(input, "regedit")) {
			cNetwork::Shell_Command("regedit");
		} else if (!openw3d::stricmp(input, "logfile")) {
   ...
   virtual	const char * Get_Alias( void ) override { return "rm"; }
	...
	virtual	void Activate( const char * input ) override {
		if (COMBAT_SCENE != NULL) {
			if ((openw3d::stricmp(input,"points") == 0) || (openw3d::stricmp(input,"2") == 0)) {
				COMBAT_SCENE->Set_Polygon_Mode(SceneClass::POINT);
				Print("Polygon mode set to POINT\n");
			} else if ((openw3d::stricmp(input,"lines") == 0) || (openw3d::stricmp(input,"1") == 0)) {
				COMBAT_SCENE->Set_Polygon_Mode(SceneClass::LINE);
				Print("Polygon mode set to LINE\n");
			} else if ((openw3d::stricmp(input,"fill") == 0) || (openw3d::stricmp(input,"0") == 0)) {
...
public:
   ...
   virtual	const char * Get_Alias( void ) override { return "rm"; }
	...
	virtual	void Activate( const char * input ) override {
		if (COMBAT_SCENE != NULL) {
			if ((openw3d::stricmp(input,"disable") == 0) || (openw3d::stricmp(input,"0") == 0)) {
			...
			} else if ((openw3d::stricmp(input,"zbuffered") == 0) || (openw3d::stricmp(input,"2") == 0)) {
...
public:
	...
	virtual	void Activate( const char * input ) override {
		if (openw3d::stricmp(input,"none") == 0) {
...
public:
	...
	virtual	void Activate( const char * input ) override {
		...
		else if (openw3d::stricmp(input,"opaque") == 0) {
			DX8Wrapper::_Enable_Triangle_Draw(true);
			SortingRendererClass::_Enable_Triangle_Draw(false);
			Print("Only opaque polygons enabled\n");
		}
		else if (openw3d::stricmp(input,"sorting") == 0) {
			DX8Wrapper::_Enable_Triangle_Draw(false);
...
public:
   ...
   const char * Get_Alias( void ) override { return "dt"; }
	...
	virtual	void Activate( const char * input ) override {
		char str[128];
		sprintf(str,"ERROR (%s)\n", input );
		if (openw3d::stricmp(input,"info") == 0) {
		...
		} else if (openw3d::stricmp(input,"warning") == 0) {
		...
		} else if (openw3d::stricmp(input,"error") == 0) {
		...
		} else if (openw3d::stricmp(input,"script") == 0) {
		...
		} else if (openw3d::stricmp(input,"netadmin") == 0) {
		...
		} else if (openw3d::stricmp(input,"netbasic") == 0) {
		...
		} else if (openw3d::stricmp(input,"netprolific") == 0) {
		...
		} else if (openw3d::stricmp(input,"net") == 0) {
...
public:
	...
	virtual void Activate(const char * input) override {
		if (!openw3d::stricmp(input, "success")) {
			CombatManager::Mission_Complete(true);
		} else if (!openw3d::stricmp(input, "failure")) {
...
public:
   ...
   const char * Get_Alias( void ) override { return "dd"; }
	...
	virtual	void Activate( const char * input ) override {
		char str[128];
		sprintf(str,"ERROR (%s)\n", input );
		if (openw3d::stricmp(input,"screen") == 0) {
		...
		} else if (openw3d::stricmp(input,"mono") == 0) {
		...
		} else if (openw3d::stricmp(input,"dbwin32") == 0) {
		...
		} else if (openw3d::stricmp(input,"log") == 0) {
		...
		} else if (openw3d::stricmp(input,"windows") == 0) {
	...
	bool KickGameSpyUser(const WideStringClass & user_name) {
		if (cGameSpyAdmin::Is_Gamespy_Game()) {
			...
			for (SLNode<cPlayer> *player_node = cPlayerManager::Get_Player_Object_List ()->Head ()
				...
				if (openw3d::stricmp(StringClass(user_name), StringClass(player->Get_Name())) == 0) {
	...
	virtual	void Activate( const char * input ) override {
		...
		if ((cGameSpyAdmin::Is_Gamespy_Game() || is_wol) && cNetwork::I_Am_Server()) {
			...
			for (player_node = cPlayerManager::Get_Player_Object_List ()->Head ()
				...
				if (openw3d::stricmp(StringClass(input), StringClass(player->Get_Name())) == 0) {
					if (is_wol) {
...
public:
	...
	virtual	void Activate (const char *input) override {
		...
		} else if (!::openw3d::stricmp(input, "off")) {
...
void	ConsoleFunctionManager::Sort_Function_List( void )
{
	...
	while ( !done ) {
		...
		for (	int index = 0; index < count; index++) {
			if ( openw3d::stricmp(	FunctionList[index]->Get_Name(),
	...
	if ( ( function_name == NULL ) || ( function_name[0] == 0 ) ) {
      ...
      Print( "------ Available commands -------\n" );
		*string = 0;
		...
		for (	int index = 0; index < FunctionList.Count(); index++) {
			ConsoleFunctionClass * function = FunctionList[index];
			if ( !openw3d::stricmp( function->Get_Name(), function_name ) ) {
...
int ConsoleFunctionManager::Find_Function_Node(const char * name)
{
	for (	int index = 0; index < FunctionList.Count(); index++) {
		ConsoleFunctionClass * function = FunctionList[index];
		if (openw3d::stricmp(name,function->Get_Name()) == 0) {
			return index;
		}
	}
	return -1;
