...
const char *BuildInfoClass::Get_Builder_Initials(void)
{
	...
	_buffer[0] = *GitCommitAuthorName;
	_buffer[1] = 0;
	_buffer[2] = 0;

	const char *lastname = strchr(GitCommitAuthorName, ' ');
	if (lastname) {
...
const char *BuildInfoClass::Composite_Build_Info(void)
{
	...
	sprintf(_buffer, "%s Commit %s by %s at %s", Get_Build_Type_String(), GitShortSHA1, GitCommitAuthorName, GitCommitDate);
	return(_buffer);
}
